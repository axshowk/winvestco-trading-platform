...
public class FundsEventPublisher {
        ...
        /**
         * Publish FundsLockedEvent with trading details for order processing.
         * This version includes symbol, side, orderType, quantity, and price
         * needed by trade-service to create trades.
         */
        public void publishFundsLockedWithDetails(Long userId, Wallet wallet, FundsLockDTO lock,
                ...
                log.info("Publishing FundsLockedEvent with trading details for order: {}", lock.getOrderId());
                rabbitTemplate.convertAndSend(
                                RabbitMQConfig.FUNDS_EXCHANGE,
                                RabbitMQConfig.FUNDS_LOCKED_ROUTING_KEY,
                                event);
        ...
        /**
         * Publish OrderRejectedEvent when funds locking fails (e.g., insufficient
         * funds).
         * This notifies the order-service to reject the order and the user.
         */
        public void publishOrderRejected(String orderId, Long userId, String symbol,
                ...
                log.info("Publishing OrderRejectedEvent for order: {} - {}", orderId, rejectionReason);
                rabbitTemplate.convertAndSend(
